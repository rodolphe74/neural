...
public class NetworkDrawer {
	...
	static void draw(Network network, String filename) throws IOException {
		...
		Graphics2D vg2d = new SVGGraphics2D(1000, 1000);
		...
		for (Layer l : network.getLayers()) {
			for (Neuron n : l.getNeurons()) {
				for (Synapse s : n.getOutputList()) {
					...
					vg2d.draw(new Line2D.Double(leftCircleIntersectionX, leftCircleIntersectionY,
		...
		SVGUtils.writeToSVG(f, ((SVGGraphics2D) vg2d).getSVGElement());
	}
}
